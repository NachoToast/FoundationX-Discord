...
/** A utility class for nested validating of object fields. */
export class Validator<TObject, TKey extends keyof TObject> {
     ...
     */
    public forEach<TArrayItem = TObject[TKey] extends (infer T)[] ? T : never>(
        expectedTypes: PossibleType[],
        callbackFn?: (validator: Validator<TArrayItem[], number>) => void,
    ): this {
        if (!Array.isArray(this.value)) {
            throw new Error('Value is not an array');
        }

        const typedValues = this.value as TArrayItem[];

        const validate = Validator.for<TArrayItem[]>(
            typedValues,
            this.objectName,
        );
    ...
    private checkType(expectedTypes: PossibleType[]): this {
        ...
        if (this.value === undefined) {
            ...
            isValid = (expectedTypes as string[]).includes(reportedType);
        }
